...
public class RestfulHttpClient {
    private static Logger logger = LoggerFactory.getLogger(RestfulHttpClient.class);
...
//            }
    ...
    private static HttpResponse request(HttpClient client) throws IOException {
    	logger.debug("进入RestfulHttpClient_request:::::::client:::::::::"+client.getUrl());
        HttpURLConnection con = instance(client);
        if (METHOD_GET.equalsIgnoreCase(client.getMethod())) {
            //GET请求，不用发请求体
        	logger.debug("进入RestfulHttpClient_request:::::::Get");
            return readResponse(con);
        }
        logger.debug("进入RestfulHttpClient_request:::::::POST");
        ...
        bw.close();
        logger.debug("进入RestfulHttpClient_request:::::::con::::::::"+con.toString());
        return readResponse(con);
    }
