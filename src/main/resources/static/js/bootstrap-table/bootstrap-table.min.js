...
! function(a) {
	...
	n.DEFAULTS = {
		...
		this.options.trimOnSearch && a(b.currentTarget).val() !== c && a(b.currentTarget).val(c), c !== this.searchText && (this.searchText = c, this.options.pageNumber = 1, this.initSearch(), jdbcDao.updatePagination(), this.trigger("search", c))
		...
		c.parent().addClass("active").siblings().removeClass("active"), this.options.pageSize = c.text().toUpperCase() === this.options.formatAllRows().toUpperCase() ? this.options.formatAllRows() : +c.text(), this.$toolbar.find(".page-size").text(this.options.pageSize), jdbcDao.updatePagination(b)
	}, n.prototype.onPageFirst = function(a) {
		this.options.pageNumber = 1, jdbcDao.updatePagination(a)
	}, n.prototype.onPagePre = function(a) {
		this.options.pageNumber--, jdbcDao.updatePagination(a)
	}, n.prototype.onPageNext = function(a) {
		this.options.pageNumber++, jdbcDao.updatePagination(a)
	}, n.prototype.onPageLast = function(a) {
		this.options.pageNumber = this.totalPages, jdbcDao.updatePagination(a)
	}, n.prototype.onPageNumber = function(b) {
		this.options.pageNumber !== +a(b.currentTarget).text() && (this.options.pageNumber = +a(b.currentTarget).text(), jdbcDao.updatePagination(b))
		...
		}), jdbcDao.updateSelected(), this.resetView(), this.trigger("post-body")
		...
		var b;
		a || (b = this.getSelections()), this.$selectAll.add(this.$selectAll_).prop("checked", a), this.$selectItem.filter(":enabled").prop("checked", a), jdbcDao.updateRows(), a && (b = this.getSelections()), this.trigger(a ? "check-all" : "uncheck-all", b)
	}, n.prototype.check = function(a) {
		this.check_(!0, a)
	}, n.prototype.uncheck = function(a) {
		this.check_(!1, a)
	}, n.prototype.check_ = function(a, b) {
		this.$selectItem.filter(c('[data-index="%s"]', b)).prop("checked", a), this.data[b][this.header.stateField] = a, jdbcDao.updateSelected(), this.trigger(a ? "check" : "uncheck", this.data[b])
		...
		if (d.hasOwnProperty("field") && d.hasOwnProperty("values")) {
			...
			}), jdbcDao.updateSelected(), this.trigger(b ? "check-some" : "uncheck-some", f)
		}
		...
		this.options.pagination ? a.attr("class", this.options.iconsPrefix + " " + this.options.icons.paginationSwitchDown) : a.attr("class", this.options.iconsPrefix + " " + this.options.icons.paginationSwitchUp), jdbcDao.updatePagination()
		...
		this.filterColumns = a.isEmptyObject(b) ? {} : b, this.options.pageNumber = 1, this.initSearch(), jdbcDao.updatePagination()
		...
		a > 0 && a <= this.options.totalPages && (this.options.pageNumber = a, jdbcDao.updatePagination())
	}, n.prototype.prevPage = function() {
		this.options.pageNumber > 1 && (this.options.pageNumber--, jdbcDao.updatePagination())
	}, n.prototype.nextPage = function() {
		this.options.pageNumber < this.options.totalPages && (this.options.pageNumber++, jdbcDao.updatePagination())
