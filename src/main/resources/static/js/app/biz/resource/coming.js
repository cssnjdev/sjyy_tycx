$(function () {
    $.post(ctx + 'resource/getResourceComing', {}, function (r) {
        if (r.code === 0) {
            var data = JSON.parse(r.msg);
            var resource_list = data.resourcecomings;
            ...
            for (var i = 0; i < resource_list.length; i++) {
                ...
                resource_list_html += '<li class="movie-img aspectration" data-ratio="16:9" onclick="getMoiveDetail(\'' + resource_list[i].id + '\');" style="background-image:url(' + resource_list[i].image + ');"></li>';
                resource_list_html += '<div class="groups__info">';
                resource_list_html += '<strong>' + resource_list[i].title + '</strong>';
                resource_list_html += '<small>' + resource_list[i].type + '</small><br/>';
                resource_list_html += '<small>' + resource_list[i].releaseDate + '</small>';
                ...
                resource_list_html += '<a class="dropdown-item" onclick="getMoiveComments(\'' + resource_list[i].id + '\',\'' + resource_list[i].title + '\');" href="javascript:void(0)">查看评论</a>';
                resource_list_html += '</div></div></div></div>';
            }
