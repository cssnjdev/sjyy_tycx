...
$(function(){
	...
	window.sqlxh = getQueryString("sqlxh");
...
function setJlValue(sjl,tj,tjuuid){
	...
	$("#tj_"+tj).empty();
	//$("#tj_"+tj).append('<option value="10299"  >'+'营业税'+'</option>');
	...
	function getCombData(tj){
		...
		$.ajax({
		      ...
		      success:function(data,textStatus,jqXHR){  
		    	 // alert(JSON.stringify(data))
	 	      	  if(data){ 
	 	      		$inp.append('<option selected="selected" style="display:none" value="'+''+'"  >'+''+'</option>');
		 	      		for(var i=0;i<data.length;i++){ 
		 	      			var dat = data[i]; 
		 	      			if(dat.dm == mrz){
...
function getColumns(){
	...
	$.ajax({
	      url:ctx+"tykf/request?tld=Tycx002DzcxService_getCxjgl",
	      async:false,
	      data:{
		    'sqlxh':sqlxh,
		    'queryType':'1',
	...
	if(Cols){
		columnss.push({title:'序号',formatter:function(value,row,index){
			var page=$('#tb_departments').bootstrapTable('getPage');
			var totalRows=$('#tb_departments').bootstrapTable('getOptions').totalRows;
			if(page.pageSize==index||page.pageSize*(page.pageNumber-1)+index==totalRows){
				return '合计';
			}else{
		...
		$(Cols).each(function(){
			...
			var column = {
				    ...
				    if(col.llx=='NUMBER'){
				    	...
				    	if(col.xsgs=='YYYY-MM-DD'||col.xsgs=='Y-m-d'){
				    		var ss=new Date(value);
				    		var year=ss.getFullYear();
				    		var month=ss.getMonth();
				    		var day=ss.getDate();
				    ...
				    if(col.url){
						...
						return "<a href='' onclick=\"tycx_onclick('"+col.url+"','"+cs+"')\"> <div class='tableCell' onmouseenter='showCellValue(this)'  style='min-width:100%;max-width:200px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;'> "+value+"</div></a>";

				    }else{
//				    	if(col.ycbj=='0'){
//				    		//隐藏
//							return " <div class='tableCell' style='display:none' onmouseenter='showCellValue(this)'  style='min-width:100%;max-width:200px;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;'> "+value+"</div>";
//
//				    	}else{
...
function tycx_onclick(url,param){
	if(url.indexOf("http")<0){
		var host = window.location.host;
		url="http://" + host + url;
	}
	url = url + '&' + param+'&isjm=0'+"&execute=true";
...
//		 var queryParamsStr = JSON.stringify(cxtjParams);
//	   $.ajax({
//		      url:ctx+"tykf/request?tld=Tycx002DzcxService_executeQuery",
//		      async:false,
//		      data:{
...
//		     "page":1,
//		     "limit":1,
//		     "summaryParams":JSON.stringify(tjparams)
//		      },
//		      dataType:'json',
//		      success:function(data,textStatus,jqXHR){  
//		    	  debugger;
...
//		 });
	  ...
	  $('#tb_departments').bootstrapTable({
		   url:'/tykf/request?tld=Tycx002DzcxService_executeQuery&a='+Math.random(),
		 // data:resultData,
		   method: 'get',         //请求方式（*）
